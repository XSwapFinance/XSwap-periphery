...
contract Swap is Base, IXSwapCallback {
    ...
    /// @param _factory address of XSwapFactory
    ...
    function swapDesireInternal(
        ...
        if (tokenOut < tokenIn) {
            ...
            (acquire, ) = IXSwapPool(poolAddr).swapY2XDesireX(
            ...
            // tokenOut is tokenY
            // tokenIn is tokenX
            (, acquire) = IXSwapPool(poolAddr).swapX2YDesireY(
                recipient, uint128(desire), -800001,
                abi.encode(data)
            );
        }
    ...
    function swapAmountInternal(
        ...
        while (true) {
            ...
            if (tokenIn < tokenOut) {
                // swapX2Y
                uint256 costX;
                (costX, acquire) = IXSwapPool(poolAddr).swapX2Y(
                ...
                // swapY2X
                uint256 costY;
                (acquire, costY) = IXSwapPool(poolAddr).swapY2X(
                    hasMultiplePools? address(this): recipient, amount, 799999,
                    abi.encode(SwapCallbackData({path: abi.encodePacked(tokenIn, fee, tokenOut), payer: payer}))
                );
                if (firstHop) {
    ...
    function swapY2X(
        ...
        (uint256 amountX, ) = IXSwapPool(poolAddr).swapY2X(
    ...
    function swapY2XDesireX(
        ...
        (amount.amountX, amount.amountY) = IXSwapPool(poolAddr).swapY2XDesireX(
            recipient, swapParams.amount, swapParams.boundaryPt,
    ...
    function swapX2Y(
        ...
        address payer = msg.sender;
        address recipient = (swapParams.recipient == address(0)) ? address(this): swapParams.recipient;
        (, uint256 amountY) = IXSwapPool(poolAddr).swapX2Y(
            recipient, swapParams.amount, swapParams.boundaryPt,
            abi.encode(SwapCallbackData({path: abi.encodePacked(swapParams.tokenX, swapParams.fee, swapParams.tokenY), payer: payer}))
        );
    ...
    function swapX2YDesireY(
        ...
        address recipient = (swapParams.recipient == address(0)) ? address(this): swapParams.recipient;
        ExchangeAmount memory amount;
        (amount.amountX, amount.amountY) = IXSwapPool(poolAddr).swapX2YDesireY(
            recipient, swapParams.amount, swapParams.boundaryPt,
            abi.encode(SwapCallbackData({path: abi.encodePacked(swapParams.tokenY, swapParams.fee, swapParams.tokenX), payer: payer}))
        );
