...
contract LimitOrderManager is Base, IXSwapAddLimOrderCallback {
    ...
    function decLimOrder(
        ...
        if (order.sellXEarnY) {
            (actualDeltaRefund, ) = IXSwapPool(pool).decLimOrderWithX(order.pt, actualDelta);
        } else {
            (actualDeltaRefund, ) = IXSwapPool(pool).decLimOrderWithY(order.pt, actualDelta);
        }
    ...
    function collectLimOrder(
        ...
        IXSwapPool(pool).collectLimOrder(recipient, order.pt, actualCollectDec, actualCollectEarn, order.sellXEarnY);
